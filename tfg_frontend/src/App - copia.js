...
// Versión: DEPLOY_PRODUCTION_READY_V5.0

import React, { useState, useEffect } from 'react'; 
import './App.css'; 

// --- CONFIGURACIÓN DE URL PARA DESPLIEGUE ---
// Aquí definimos la dirección de tu servidor en la nube.
// Al usar la variable, no tendrás que cambiar el link en 10 sitios diferentes.
const API_URL = 'https://api-tfg-cristina.onrender.com'; 
...
function App() {
  ...
  const actualizarPuntosGlobalesBD = (nuevosPuntos) => {
      ...
      if (usuario) {
          // ACTUALIZADO: Usa API_URL
          fetch(`${API_URL}/api/actualizar-puntos/`, {
              method: 'POST', headers: {'Content-Type': 'application/json'},
  ...
  const cargarSiguienteEjercicio = async (tipo) => {
      ...
      try {
          if (listaModelosDisponibles.length === 0) {
             // ACTUALIZADO: Usa API_URL
             const resModelos = await fetch(`${API_URL}/api/lista-modelos/`);
             if (resModelos.ok) {
          ...
          // ACTUALIZADO: Usa API_URL
          const res = await fetch(`${API_URL}/api/ejercicio-aleatorio/`, {
              method: 'POST', headers: {'Content-Type': 'application/json'},
  ...
  const handleSubmitResolucion = async (ecuacionStr) => {
    try {
      // ACTUALIZADO: Usa API_URL
      const res = await fetch(`${API_URL}/api/resolver/`, {
        method: 'POST', headers: {'Content-Type': 'application/json'},
  ...
  const completarEjercicioYContinuar = async () => {
      if (usuario) {
          // ACTUALIZADO: Usa API_URL
          await fetch(`${API_URL}/api/marcar-completado/`, {
              method: 'POST', headers: {'Content-Type': 'application/json'},
          ...
          <div style={{marginTop:'50px', border:'1px solid #61dafb', padding:'30px', borderRadius:'10px', backgroundColor:'#f9f9f9'}}>
              ...
              <div style={{marginTop:'30px'}}>
                  ...
                  <table style={{width:'100%', maxWidth:'400px', margin:'auto', borderCollapse:'collapse'}}>
                      ...
                      <tbody>
                          {rankingData.map((r, i) => (
                              <tr key={i} style={{borderBottom:'1px solid #ddd', color:'#333'}}>
